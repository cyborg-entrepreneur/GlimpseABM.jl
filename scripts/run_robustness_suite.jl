...
#
# Usage:
# julia --threads=auto --project=. scripts/run_reviewer_robustness_suite.jl --list
# SUITE_PRESET=minimal julia --threads=auto --project=. scripts/run_reviewer_robustness_suite.jl
# N_AGENTS=2000 N_RUNS=50 SUITE_PRESET=core julia --threads=auto --project=. scripts/run_reviewer_robustness_suite.jl
# CONDITIONS=BASELINE,NO_CROWDING N_AGENTS=64 N_RUNS=2 N_ROUNDS=6 julia --project=. scripts/run_reviewer_robustness_suite.jl
...
# Heavy-tail ("unicorn") baseline: the venture-realistic heavy
...
const UNICORN_TAIL = lowercase(get(ENV, "UNICORN_TAIL", "on")) != "off"
...
const OUTPUT_DIR = get(ENV, "OUTPUT_DIR",
    joinpath(@__DIR__, "..", "results", "reviewer_robustness_$(Dates.format(now(), "yyyymmdd_HHMMSS"))"))
...
# Heavy-tail robustness setters. Tail setters override the unicorn baseline fields;
# OPS setters override the unicorn baseline OPS_COST_INTENSITY; exec setters are independent.
...
truncated_tail!(config::EmergentConfig) = (config.LOG_SIGMA_MULT = 1.0; config.LOG_SIGMA_CAP = 1.0; config.RETURN_RANGE_MAX_MULT = 1.0; config.RETURN_CLAMP_MAX = 25.0; nothing)
moderate_tail!(config::EmergentConfig)  = (config.LOG_SIGMA_MULT = 2.5; config.LOG_SIGMA_CAP = 2.0; config.RETURN_RANGE_MAX_MULT = 50.0; config.RETURN_CLAMP_MAX = 500.0; nothing)
ops_cost_060!(config::EmergentConfig) = (config.OPS_COST_INTENSITY = 0.60; nothing)
ops_cost_085!(config::EmergentConfig) = (config.OPS_COST_INTENSITY = 0.85; nothing)
...
# Token-core migration: the subscription-era architecture is the
...
# R2's "would an AGI-level system persist in those errors?" with a curve
# (pre-registered P1–P3); ALL_* is the reflexivity-relocation test
# (pre-registered P4).
...
# action extension). A1 = abandonment option (per-round pivot review at an
...
# AGI-robustness cell, additionally the strategy ladder. Pre-registered
# P6–P8.
# PIVOT_DETERIORATION_GAIN = 2.0 in every pivot-enabled cell: liveness-
# calibrated by a liveness probe (gain 1.0 = dead arm — the
...
# The the cluster ladder run (results/reviewer_robustness_20260610_004356) left the
# frontier trap (−18.5pp) essentially unmoved at the documented default
# strengths (attenuation 0–1.4pp; pre-registered P1 falsified) and the pivot
# arm nearly dead at N=1000 despite gain 2.0 being liveness-calibrated at
# N=64 (deterioration swings compress with population size). The reviewer
# question those results leave open: is the trap strategy-PROOF (no
...
# range into the trigger region (P6 scale recalibration: the N=64
...
function dose_open_composite_max!(config::EmergentConfig)
 # Maximal-dose version of the P8 cell: pivot at gain 8.0, directed
 # creation at its DEFAULT mixture strength (A2 was never the gentle
 # dial — keeping it pinned preserves comparability with P7/P8), and
...
# Two mechanism dials motivated by the post-the cluster flatness question (the TE
...
function build_config(condition::RobustnessCondition, seed::Int)
    ...
    if UNICORN_TAIL
 # Heavy-tail baseline applied to EVERY condition BEFORE its mutator,
...
function summarize_simulation(condition::RobustnessCondition, sim::EmergentSimulation, run_idx::Int, seed::Int)
 ...
 # Population-level end-of-run measures so the pre-registered P6 (pivot)
 # and P7 (directed creation / supply elasticity) predictions are evaluable
 ...
 # P7 supply-elasticity regression input: mean perceived crowding × spawn
...
function all_conditions()
 ...
 # --- Heavy-tail robustness additions: defensibility dose-response (P19),
 # tail-heaviness robustness (P20), cost-calibration invariance (P21). ---
        ...
        RobustnessCondition(
            ...
            "Conservative robustness: recover the truncated opportunity-return distribution (~4x cap).",
            "P20: the truncated distribution is the conservative case; the trap should persist.",
            "fixed_mixed", "expanded", truncated_tail!),
        RobustnessCondition(
            "MODERATE_TAIL", "opportunity_distribution",
            "Intermediate tail (sigma x2.5, ~100x ceiling) between truncated and the unicorn baseline.",
            "P20: tail dose-response; the trap is robust across tail weight.",
        ...
        RobustnessCondition(
            "OPS_COST_085", "cost_calibration_invariance",
            "Operating-cost level 0.85.",
            "P21 cost-calibration invariance.",
            "fixed_mixed", "expanded", ops_cost_085!),
        RobustnessCondition(
        ...
        RobustnessCondition(
            ...
            "independent-implementation robustness check for the canonical S1/ladder operationalization (see the design notes Canonicality)",
 ...
 # Token-core migration: pure usage-based pricing is now
 ...
 # ── AGI strategy ladder (the design notes) ──
        RobustnessCondition(
            ...
            "Trap attenuation, congestion channel (R2's question): does the frontier deficit shrink when its users discount what every rival's instrument also ranks highly? Pre-registered P1 expects S1 to carry the largest single share.",
        ...
        RobustnessCondition(
            ...
            "The strongest ecologically defensible operationalization of R2's 'an AGI-level system would update'; pre-registered P3 predicts relative advantage still does not return beyond the FREE_AI bound.",
        ...
        RobustnessCondition(
            ...
            "Reflexivity relocation test (pre-registered P4): when contrarianism is the consensus, does crowding relocate into formerly-neglected opportunities or retire?",
        ...
        RobustnessCondition(
            ...
            "The AGI-robustness test (pre-registered P4): does the paradox of future knowledge survive population-wide AGI-native strategy one level up, or does reflexivity retire under symmetric sophistication?",
            "fixed_mixed", "expanded", all_strategy_composite!),

 # ── Open-action extension (the design notes) ──
        RobustnessCondition(
            "OPEN_ACTION_PIVOT", "open_action_space",
            "Abandonment option (A1): agents review in-flight investments each round and may liquidate at an age-dependent haircut (0.40 → 0.75 of committed capital), redeploying the recovery. Deterioration gain 2.0 per liveness probe.",
            "Pre-registered P6: pivot attenuates the trap (escape from crowded commitments) but does not eliminate it; pivot re-correlation onto consensus second-bests is possible and reported either way. Falsifier: trap unchanged or eliminated.",
        ...
        RobustnessCondition(
            ...
            "Pre-registered P7: directed creation raises measured supply elasticity above the ≈0 anti-Hayekian baseline and shrinks the trap; second-order crowding of formerly-sparse sectors may emerge (reflexivity relocating). Falsifier: elasticity unchanged, or trap eliminated by redirection alone.",
 ...
 # comment above the apply! functions).
        ...
        RobustnessCondition(
            ...
            "Abandonment option at deterioration gain 4.0 — 2x the N=64 liveness-calibrated 2.0. Gain is validated >= 0 and unbounded above; the trigger d_eff = clamp(deterioration x conviction x gain, 0, 1). P6 scale recalibration: per-agent perceived-crowding deterioration swings compress from N=64 to N=1000, so the gain that made the arm live in the probe (~3-5% pivot rates) left it nearly dead at production scale.",
            "Dose-response for the pivot channel: locates the gain that restores arm liveness at N=1000, bounding P6's attenuation curve — a dead arm at default gain says nothing about whether exit options attenuate the trap.",
        ...
        RobustnessCondition(
            ...
            "Maximal-dose version of the P8 cell: pivot at gain 8.0 + directed creation (default mixture strength — A2 was not the gentle dial; pinned for P7/P8 comparability) + population-wide agi_native at max-effective strengths (discount 1.0 / edge 2.0 / shift 2.0).",
            "Dose-response ceiling for P8: if the trap survives the maximal expressible open-action + strategy dose, strategy-proof is the verdict at the strongest claim level; if it folds only here, the dose-response sweep has located the boundary of the paradox.",
 ...
 # §Emergence audit extension; pre-registered P9/P10) ──────────────
        RobustnessCondition(
            ...
            "Pre-registered P9 (quality-vs-commonality decomposition): at fixed accuracy the frontier TE deepens monotonically in rho, toward the NO_AI_ERRORS bound (~-27pp) at unchanged accuracy — the trap tracks signal CORRELATION, not signal QUALITY. The midpoint locates curvature of the rho dose-response. Falsifier: TE flat in rho.",
        ...
        RobustnessCondition(
            ...
            "Pre-registered P9 endpoint (quality-vs-commonality decomposition): prediction is the TE deepens toward the NO_AI_ERRORS bound (~-27pp) at unchanged accuracy, establishing that the default fully-idiosyncratic error structure was the conservative, trap-MINIMIZING case. Falsifier: TE flat in rho — convergence would then not be signal-correlation-driven and the mechanism claim must be revised.",
        ...
        RobustnessCondition(
            ...
            "Pre-registered P10, deterministic arm: lowering T (toward argmax, the deterministic-AI limit) deepens the trap — determinism strips out the mixing that decorrelates entry. Together with DECISION_NOISE_HIGH this brackets the editor's conjecture in both directions. Falsifier: no response in either direction.",
 ...
 # converges to, and which mechanism levers move that mix. Pre-registered
 # P12-P14 (the design notes). Run with
 ...
 # ── Fixed-tier robustness addendum ───────────────────────
        RobustnessCondition(
...
function selected_conditions()
 ...
 # adoption-addendum preset so the expanded manifest stays identical to
 # the prior run; the addendum runs as its own job.
...
function main()
 ...
 # Added 2026-06-10 after job 5643202 failed exactly this way: the
 # emergence-audit conditions were synced to the cluster without the src/
 # fields they set. Costs <1s for the full condition set.
    ...
    mkpath(per_cond_dir)
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_manifest.csv"),
 ...
 # space) — verified to parse and run on local Julia 1.12.3; cluster runs
 ...
 # multi-day cluster run must never discard the raw results. (The manifest was
 # already written before the loop.)
    per_run_df = DataFrame(all_rows)
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_per_run.csv"), per_run_df)
    println("[checkpoint] raw results written: reviewer_suite_per_run.csv " *
    ...
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_summary.csv"), summary_df)
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_paired_treatment_effects.csv"), paired_df)
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_delta_vs_baseline.csv"), delta_df)
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_paradox_scorecard.csv"), scorecard_df)
    CSV.write(joinpath(OUTPUT_DIR, "reviewer_suite_paradox_scorecard_delta_vs_baseline.csv"), scorecard_delta_df)
